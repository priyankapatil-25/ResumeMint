...
export async function PUT(req: Request) {
  try {
    ...
    await prisma.user.update({
      ...
      data: {
        name: data.name, phone: data.phone, address: data.address, photo: data.photo,
    ...
    if (data.certifications !== undefined) {
      ...
      if (data.certifications.length > 0)
        await prisma.certification.createMany({ data: data.certifications.map((c: { title: string; issuer: string; date: string; url: string }) => ({ title: c.title, issuer: c.issuer || "", date: c.date || "", url: c.url || "", userId: uid })) });
    }
