  ...
  return (
    <div className="flex flex-col items-center gap-1">
      <svg width="44" height="44" className="transform -rotate-90">
        <circle cx="22" cy="22" r={r} fill="none" stroke="rgba(99,102,241,0.15)" strokeWidth="3" />
  ...
  return (
    <div className="flex items-center gap-3 mb-4 mt-8">
      <div className="w-8 h-8 rounded-lg flex items-center justify-center"
        style={{ background: "linear-gradient(135deg, rgba(99,102,241,0.3), rgba(45,212,191,0.2))" }}>
        {icon}
      </div>
  ...
  return (
    <div className="min-h-[297mm] w-[210mm] mx-auto relative overflow-hidden"
      ...
      <div className="absolute bottom-20 left-0 w-[250px] h-[250px] rounded-full opacity-15 pointer-events-none"
        style={{ background: "radial-gradient(circle, rgba(45,212,191,0.3) 0%, transparent 70%)", filter: "blur(50px)" }} />

      {/* ===== HEADER ===== */}
      <div className="relative px-10 pt-8 pb-7"
        style={{ background: "linear-gradient(135deg, rgba(99,102,241,0.12) 0%, rgba(45,212,191,0.08) 50%, rgba(99,102,241,0.05) 100%)" }}>
        {/* Gradient border bottom */}
        <div className="absolute bottom-0 left-0 right-0 h-[2px]"
          style={{ background: "linear-gradient(90deg, #6366F1, #2DD4BF, #6366F1)" }} />
          ...
          <div className="relative shrink-0">
            <div className="w-[95px] h-[95px] rounded-2xl overflow-hidden flex items-center justify-center text-white text-3xl font-bold"
              style={{
                ...
                boxShadow: "0 0 0 3px rgba(99,102,241,0.3), 0 8px 32px rgba(0,0,0,0.4)",
              }}>
              {profile.photo ? (
            ...
            <div className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full border-[3px] flex items-center justify-center"
              style={{ borderColor: "#060B18", background: "linear-gradient(135deg, #6366F1, #2DD4BF)" }}>
            ...
            {profile.branch && (
              <div className="flex items-center gap-2 mt-2">
                <div className="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider"
                  style={{ background: "linear-gradient(135deg, rgba(99,102,241,0.25), rgba(45,212,191,0.15))", color: "#A5B4FC", border: "1px solid rgba(99,102,241,0.2)" }}>
                  {profile.branch}
                </div>
                {cgpa !== "N/A" && (
        ...
        {profile.objective && (
          <>
            ...
            <div className="rounded-xl p-4 text-[11px] leading-[1.8] text-slate-300 italic"
              style={{ background: "linear-gradient(135deg, rgba(30,41,59,0.6), rgba(30,41,59,0.3))", border: "1px solid rgba(99,102,241,0.1)", borderLeft: "3px solid #6366F1" }}>
        ...
        {profile.college && (
          <>
            ...
            <div className="rounded-xl p-5 relative overflow-hidden"
              style={{ background: "linear-gradient(135deg, rgba(30,41,59,0.7), rgba(15,23,42,0.5))", border: "1px solid rgba(99,102,241,0.15)" }}>
              {/* Decorative corner */}
              <div className="absolute top-0 right-0 w-20 h-20 opacity-30 pointer-events-none"
                style={{ background: "radial-gradient(circle at 100% 0%, rgba(99,102,241,0.4), transparent 70%)" }} />
              <div className="flex items-start justify-between relative">
                ...
                <div className="text-right shrink-0 ml-4">
                  ...
                  {cgpa !== "N/A" && (
                    <p className="text-[16px] font-extrabold mt-2" style={{ color: "#2DD4BF" }}>
        ...
        {(profile.school10th || profile.school12th) && (
          <>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { label: "XII Standard", school: profile.school12th, board: profile.board12th, pct: profile.percentage12th, yr: profile.year12th, color: "#818CF8" },
                { label: "X Standard", school: profile.school10th, board: profile.board10th, pct: profile.percentage10th, yr: profile.year10th, color: "#2DD4BF" },
              ].map((s) =>
                s.school ? (
                  <div key={s.label} className="rounded-xl p-4 relative overflow-hidden"
                    ...
                    <div className="absolute top-0 left-0 w-full h-[3px]" style={{ background: `linear-gradient(90deg, ${s.color}, transparent)` }} />
                ...
                {profile.semesters.map((sem) => {
                  ...
                  return (
                    <div key={sem.number} className="flex flex-col items-center gap-1 flex-1">
                      <span className="text-[9px] font-bold font-mono" style={{ color: isHigh ? "#2DD4BF" : "#818CF8" }}>
                        {sem.sgpa.toFixed(1)}
                      </span>
            ...
            <div className="space-y-3">
              {profile.projects.map((project, i) => (
                <div key={i} className="rounded-xl p-4 relative overflow-hidden group"
                  style={{ background: "linear-gradient(135deg, rgba(30,41,59,0.6), rgba(15,23,42,0.4))", border: "1px solid rgba(99,102,241,0.12)" }}>
                  {/* Left accent */}
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-2.5">
                        {project.techStack.map((tech, j) => (
                          <span key={j} className="px-2.5 py-[3px] text-[9px] rounded-full font-semibold"
                            style={{ background: "linear-gradient(135deg, rgba(45,212,191,0.15), rgba(99,102,241,0.1))", color: "#5EEAD4", border: "1px solid rgba(45,212,191,0.15)" }}>
                            {tech}
                          </span>
                        ))}
                      </div>
          ...
          <>
            ...
            <div className="relative pl-5">
              ...
              <div className="absolute left-[7px] top-2 bottom-2 w-[2px]"
                style={{ background: "linear-gradient(180deg, #6366F1, #2DD4BF, transparent)" }} />
              <div className="space-y-4">
